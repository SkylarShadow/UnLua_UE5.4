...
---@field protected MeshDeformer UMeshDeformer @The mesh deformer to use. If no mesh deformer is set from here or the SkeletalMesh, then we fall back to the fixed function deformation.
...
---@field private bForceUpdateDynamicDataImmediately boolean @Whether to update dynamic bone & cloth sim data immediately, not to wait until GDME or defer update to RHIThread. When set to true, it is the equivalent of r.DeferSkeletalDynamicDataUpdateUntilGDME=0 and r.RHICmdDeferSkeletalLockAndFillToRHIThread=0. When set to false, r.DeferSkeletalDynamicDataUpdateUntilGDME and r.RHICmdDeferSkeletalLockAndFillToRHIThread values are respected.
